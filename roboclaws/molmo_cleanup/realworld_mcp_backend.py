...
from roboclaws.molmo_cleanup.realworld_mcp_promoted_tools import (
    ...
    promoted_cleanup_tool_names_for_profile,
...
def validate_realworld_mcp_tool_call(server: Any, name: str) -> None:
    ...
    if name == CLEAN_OBSERVED_OBJECT_TOOL and server.cleanup_profile != WORLD_LABELS_PERF_PROFILE:
        raise ValueError(
...
def public_tool_names_for_profile(cleanup_profile: str | None) -> tuple[str, ...]:
    return (
        *SEMANTIC_CLEANUP_TOOL_NAMES,
        *ATOMIC_CLEANUP_TOOL_NAMES,
        *promoted_cleanup_tool_names_for_profile(cleanup_profile),
        "done",
    )
...
def agent_view_public_tool_names(server: Any, base_tool_names: list[str]) -> list[str]:
    ...
    for tool in promoted_cleanup_tool_names_for_profile(server.cleanup_profile):
