...
def _cleanup_report_sections(
    *,
    ...
    _write_semantic_map_artifacts(run_dir, run_result, robot_view_steps)
        ...
        [
            ...
            _report_tab_panel(
                ...
                [
                    ...
                    _map_evidence_refresh_summary_section(run_result),
...
def _map_evidence_refresh_summary_section(run_result: dict[str, Any]) -> str:
...
def _nav2_map_bundle_section(run_dir: Path, run_result: dict[str, Any]) -> str:
    ...
    static_preview = _write_nav2_static_navigation_preview(run_dir, run_result) or artifacts.get(
        "preview_png", ""
    )
    semantic_map_path = run_dir / "semantic_map.png"
    semantic_preview = (
        _report_asset_src(semantic_map_path, run_dir) if semantic_map_path.is_file() else ""
    )
...
def _write_semantic_map_artifacts(
    run_dir: Path,
    run_result: dict[str, Any],
    robot_view_steps: list[dict[str, Any]],
) -> str:
    ...
    return _report_asset_src(semantic_map_path, run_dir)
...
def _robot_timeline(run_dir: Path, steps: list[dict[str, Any]]) -> str:
    ...
    for index, step in enumerate(steps, start=1):
        ...
        cards.append(
            ...
            f"{_view_figure(views.get('map'), 'Top-down Scene View')}"
            f"{fpv_bbox_figure}"
            "</div>"
            f"{sim_only_views}"
            "</article>"
    ...
    return (
        ...
        "surfaces. Semantic Map is rendered separately from public map/runtime data. "
