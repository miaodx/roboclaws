...
| `clean_observed_object(object, fixture)` | Performance-lane promoted candidate unless promotion criteria are met | It can reduce MCP round trips, but canonical cleanup now defaults to the skill-side trace-preserving routine and uses this tool only for explicit comparison runs. |
