...
```bash
ROBOCLAWS_JUST_TRACE=1 just task::run semantic-map-build codex camera-labels \
  backend=agibot_gdk \
...
- `--server-arg=--evidence-lane` / `--server-arg=camera-labels`
- `--server-arg=--visual-grounding` / `--server-arg=grounding-dino`
...
- route 默认带 `policy=codex_agibot_semantic_map_build_pilot`、
  `visual_grounding=grounding-dino` 和 `visual_grounding_timeout_s=20`。
- 自定义 task prompt。
...
- `camera-labels` 有成功的 External Visual Grounding Service evidence
