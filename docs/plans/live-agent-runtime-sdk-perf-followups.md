...
1. Behavior-preserving compaction/session redesign, not a rerun of the rejected
   I/N/AB policy or the latest O context-overflow row. Any new row must preserve
   `done`, report-quality evidence, and task-state semantics while attacking
   the dominant model/SDK between-tool gap and O's context growth.
