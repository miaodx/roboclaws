...
N/I/AB live A/B against the dominant model/SDK between-tool gap and toward D
before any C racing experiment, rather than another standalone F row.
...
13. Because token cost is not a deciding objective, D/C racing is no longer
    ...
    may be the next wall-clock arm if Q still shows model/SDK gap dominant and
    live run caps cover the racing multiplier. Still defer K/E/M/X unless the
    dependency evidence below appears.
...
| ID | Queue | Do | Why | Success | Stop / next |
| --- | --- | --- | --- | --- | --- |
| D | accepted-deterministic-prep, C prerequisite | Add per-arm racing observability. | Racing can look faster while hiding loser-arm work; even when token cost is not a decision objective, normalized wall-clock claims need winner/loser timing and cancellation evidence. | Implemented: each SDK model call records sanitized single-arm lifecycle events, stable call/arm ids, elapsed time, winner/cancel flags, failure class, provider/model/wire axes, aggregate token usage availability, loser-billing-unknown fields, and aggregate `model_racing_observability_metrics` in `live_timing.json` latency attribution. Current mode is single-arm/no-racing and does not enable C. | C can now be scoped as the next live arm when Q still shows model/SDK gap dominant and run caps cover the racing multiplier; keep D events mandatory for any C row. |
| C | keep-after-D | Race individual SDK model calls. | The dominant bucket is still model/SDK between-tool gap, and token cost is not a deciding objective, so provider tail-latency racing is a plausible wall-clock arm once attribution exists. | First schema-valid/MCP-legal response wins; only winner enters history; losers are cancelled/recorded. | Do not run until Q still shows model/SDK gap dominant after the latest D-equipped timing packet, run caps cover the racing multiplier, and label lanes are scoped. |
