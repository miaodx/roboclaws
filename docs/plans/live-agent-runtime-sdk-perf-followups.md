...
Current wall-clock priority after the O+AC repeat update:

1. Calibrate normalized model/latency evidence before any normalized or
   publishable speed claim. O+AC now has two accepted paired diagnostic rows,
   but committed latency calibration coefficients are still unavailable.
