...
Current wall-clock priority after the O+AC fixed4 update:

1. Repeat or calibrate the O+AC camera-grounded win before any normalized or
   publishable speed claim. The fixed4 row is the strongest paired diagnostic
   result so far, but it is still one seed/provider/lane row without committed
   latency calibration coefficients.
2. Scoped B baseline coverage across provider/model, `wire_api`, and evidence
...
| ID | Queue | Do | Why | Success | Stop / next |
| --- | --- | --- | --- | --- | --- |
| AC | accepted-deterministic-prep, live paired row accepted | Add completion-safe camera-grounded history compaction. | O continuation tightening removed standalone declarations but failed before completion on context budget. The rejected I/N policy compacted too broadly and broke task completion; AC narrows compaction to old camera-grounded observation/declaration history while retaining recent actionable outputs. | Implemented as `camera_grounded_history_v1` inside the private SDK model-input filter. It is off by default, can be enabled through `--camera-grounded-history-compaction` / `ROBOCLAWS_OPENAI_AGENTS_CAMERA_GROUNDED_HISTORY_COMPACTION`, keeps the latest N camera-grounded outputs full, summarizes older outputs only when smaller, records aggregate camera-grounded byte/count metrics, and preserves complete MCP traces/reports/run artifacts. The fixed4 live row completed with `done`, `run_result.json`, same-or-better quality, 14 `observe_camera_grounded_candidates` calls, 270/457 camera-grounded history items compacted, and `-659.477s` paired observed wall time. | Keep AC SDK-private/opt-in. Do not treat one fixed4 row as normalized or publishable; next proof is repeated/calibrated O+AC evidence or broader B baseline coverage. |
...
| AC | `openai-agents-live`, deterministic camera-grounded history compaction prep plus `mify` Responses O+AC live row | `model_input_compaction=camera_grounded_history_v1` opt-in through `--camera-grounded-history-compaction` / `ROBOCLAWS_OPENAI_AGENTS_CAMERA_GROUNDED_HISTORY_COMPACTION`, no public MCP/profile change | Deterministic prep evidence above; live caps `docs/status/active/agent-sdk-o-ac-camera-grounded-live-caps.json`; candidate `output/agent-sdk-perf-followups/mify-camera-grounded-mimo-compact-composite-ac-fixed4/0612_1841/seed-7/`; comparison `output/agent-sdk-perf-followups/mify-camera-grounded-composite-ac-fixed4-comparison.json`; refresh row `mify_camera_grounded_o_ac_fixed4_refresh` in `docs/status/active/agent-sdk-speedup-live-refresh-matrix.json`; `.venv/bin/python scripts/molmo_cleanup/run_agent_sdk_perf_matrix.py --manifest docs/status/active/agent-sdk-speedup-live-refresh-matrix.json --dry-run`; `.venv/bin/python scripts/molmo_cleanup/run_agent_sdk_perf_matrix.py --manifest docs/status/active/agent-sdk-speedup-live-refresh-matrix.json --offline-preflight --decision-packet output/agent-sdk-perf-followups/live-refresh-decision.json` | accepted-deterministic-prep, live paired row accepted | Deterministic proof shows the SDK model-input filter can retain the latest camera-grounded outputs while replacing older `observe_camera_grounded_candidates` / camera-model `declare_visual_candidates` / camera-policy `observe` outputs with hash/observation/candidate summaries only when smaller. The fixed4 live row preserved `done`, `run_result.json`, and same-or-better cleanup quality (`completion_status=success`, `restored_count=4/5`, `semantic_accepted_count=5/5`, `disturbance_count=0`) while reducing observed wall/model API/between-tool gap by `-659.477s` / `-653.563s` / `-638.809s`. It exercised 14 `observe_camera_grounded_candidates` calls and compacted 270 of 457 camera-grounded history items, reducing camera-grounded history bytes by `5255583`. | Keep O+AC SDK-private/opt-in and refresh Q/Y from the fixed4 row. Next recommended work is repeated/calibrated O+AC evidence or scoped B baseline coverage; normalized/publishable claims still require repeated/calibrated proof. |
