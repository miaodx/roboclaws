...
13. Because token cost is not a deciding objective, D/C racing is no longer
    deferred mainly on cost. Keep D as the required per-arm observability
    prerequisite before any C racing run. After rejected F and I/N/AB rows, D is
    the next implementation candidate if the team wants another wall-clock arm
    before tuning compaction semantics. Still defer K/E/M/X unless the dependency
    evidence below appears.
...
| ID | Queue | Do | Why | Success | Stop / next |
| --- | --- | --- | --- | --- | --- |
| D | keep, C prerequisite | Add per-arm racing observability. | Racing can look faster while hiding loser-arm work; even when token cost is not a decision objective, normalized wall-clock claims need winner/loser timing and cancellation evidence. | Timing records arm start/finish/cancel, winner, usage availability, token/cache fields, and unknown loser billing. | Do D immediately before/with C; do not run C without this attribution. |
| C | keep-after-D | Race individual SDK model calls. | The dominant bucket is still model/SDK between-tool gap, and token cost is not a deciding objective, so provider tail-latency racing is a plausible wall-clock arm once attribution exists. | First schema-valid/MCP-legal response wins; only winner enters history; losers are cancelled/recorded. | Do not run until D exists, Q still shows model/SDK gap dominant after I/N/AB, run caps cover the racing multiplier, and label lanes are scoped. |
