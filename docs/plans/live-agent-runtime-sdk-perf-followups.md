...
profiles. This is deterministic prep only; the next speed decision still needs
a provider-backed O+AC paired row that preserves `done`, `run_result.json`, and
...
14. Candidate AC deterministic prep is accepted as a narrower
    ...
    complete MCP traces/reports/run artifacts. No speed claim exists until a
    provider-backed O+AC row completes with `done`, `run_result.json`, and
    same-or-better report-quality evidence.

Current wall-clock priority after the C and raw-FPV updates:

1. Behavior-preserving compaction/session redesign, not a rerun of the rejected
   I/N/AB policy or the latest O context-overflow row. Candidate AC now provides
   the first deterministic slice of that redesign by compacting only old
   camera-grounded history while retaining recent actionable outputs. Any new
   live row must preserve `done`, report-quality evidence, and task-state
   semantics while attacking the dominant model/SDK between-tool gap and O's
   context growth.
2. O paired repeats only after the context/session redesign, because O is still
   the only live mechanism row that preserved recorded quality and reduced
   wall/model time, and continuation tightening fixed the residual standalone
   declarations but failed before completion on context budget.
...
| ID | Queue | Do | Why | Success | Stop / next |
| --- | --- | --- | --- | --- | --- |
| AC | accepted-deterministic-prep, live A/B pending | Add completion-safe camera-grounded history compaction. | O continuation tightening removed standalone declarations but failed before completion on context budget. The rejected I/N policy compacted too broadly and broke task completion; AC narrows compaction to old camera-grounded observation/declaration history while retaining recent actionable outputs. | Implemented as `camera_grounded_history_v1` inside the private SDK model-input filter. It is off by default, can be enabled through `--camera-grounded-history-compaction` / `ROBOCLAWS_OPENAI_AGENTS_CAMERA_GROUNDED_HISTORY_COMPACTION`, keeps the latest N camera-grounded outputs full, summarizes older outputs only when smaller, records aggregate camera-grounded byte/count metrics, and preserves complete MCP traces/reports/run artifacts. Focused tests cover compaction behavior, profile parsing, nested policy propagation, and aggregate metrics. | Next proof is a scoped O+AC `mify` Responses `camera-grounded-labels` paired row after normal Group 0 gates and live caps. Stop if it fails `done`/`run_result.json`, regresses cleanup quality, hides current actionable state, or requires persisting raw prompts/model text/full tool payload bodies/private truth. |
...
| AC | `openai-agents-live`, deterministic camera-grounded history compaction prep | `model_input_compaction=camera_grounded_history_v1` opt-in through `--camera-grounded-history-compaction` / `ROBOCLAWS_OPENAI_AGENTS_CAMERA_GROUNDED_HISTORY_COMPACTION`, no public MCP/profile change | `roboclaws/agents/drivers/openai_agents_live.py`, `scripts/molmo_cleanup/run_live_openai_agents_cleanup.py`, `tests/unit/agents/test_live_runtime.py`; `./scripts/dev/run_pytest_standalone.sh -q tests/unit/agents/test_live_runtime.py`; `.venv/bin/ruff check roboclaws/agents/drivers/openai_agents_live.py scripts/molmo_cleanup/run_live_openai_agents_cleanup.py tests/unit/agents/test_live_runtime.py`; `.venv/bin/ruff format --check roboclaws/agents/drivers/openai_agents_live.py scripts/molmo_cleanup/run_live_openai_agents_cleanup.py tests/unit/agents/test_live_runtime.py`; `.venv/bin/python scripts/molmo_cleanup/run_agent_sdk_perf_matrix.py --manifest docs/status/active/agent-sdk-speedup-live-refresh-matrix.json --dry-run`; `.venv/bin/python scripts/molmo_cleanup/run_agent_sdk_perf_matrix.py --manifest docs/status/active/agent-sdk-speedup-live-refresh-matrix.json --offline-preflight --decision-packet output/agent-sdk-perf-followups/live-refresh-decision.json` | accepted-deterministic-prep, live A/B pending | Deterministic proof shows the SDK model-input filter can retain the latest camera-grounded outputs while replacing older `observe_camera_grounded_candidates` / camera-model `declare_visual_candidates` / camera-policy `observe` outputs with hash/observation/candidate summaries only when smaller. Runtime config now preserves nested compaction policies, and `model_input_filter_metrics` aggregates camera-grounded history counts and byte deltas without raw prompts, model text, full tool payload bodies, credentials, private truth, or image bytes. Existing public/default profiles remain off. | Pair AC with the next O camera-grounded proof row. Do not claim speed until the row completes with `done`, `run_result.json`, same-or-better cleanup quality, and lower paired-comparable wall/model latency. |
