...
paired-comparable latency. This moves the next high-leverage work toward
...
is deterministic observability prep only; no C racing run or speed claim is
made.
...
13. Because token cost is not a deciding objective, D/C racing is no longer
    ...
    deterministic prep is now implemented behind explicit opt-in flags and live
    cap fields, but no provider-backed C row has run. The next wall-clock arm
    can be a scoped C live A/B if Q still shows model/SDK gap dominant and
    recorded caps cover `racing_multiplier=2.0` or the selected arm count. Still
    defer K/E/M/X unless the dependency evidence below appears.
...
| D | `openai-agents-live`, provider/evidence lane agnostic deterministic racing-observability prep | `model_racing_observability=per_arm_observability_v1`, single-arm/no-racing, no public MCP/profile change | `roboclaws/agents/drivers/openai_agents_live.py`, `scripts/molmo_cleanup/run_live_openai_agents_cleanup.py`, `tests/unit/agents/test_live_runtime.py`; `./scripts/dev/run_pytest_standalone.sh -q tests/unit/agents/test_live_runtime.py`; `.venv/bin/ruff check roboclaws/agents/drivers/openai_agents_live.py scripts/molmo_cleanup/run_live_openai_agents_cleanup.py tests/unit/agents/test_live_runtime.py` | accepted-deterministic-prep, C prerequisite satisfied | Deterministic proof shows SDK model calls now emit sanitized single-arm lifecycle events with stable call/arm ids, elapsed time, winner/cancel flags, failure class, provider/model/wire axes, token usage availability, and loser-billing-unknown fields. The runner aggregates these into `model_racing_observability_metrics` in `live_timing.json` timeline latency attribution. Current mode keeps `racing_enabled=false` and `racing_multiplier=1.0`, so no racing or speed claim is made. | Candidate C can now be planned as a scoped live arm if Q still shows model/SDK gap dominant and run caps cover the racing multiplier. Any C row must preserve D event evidence, privacy gates, and same-or-better behavior quality unless explicitly waived. |
| C | `openai-agents-live`, provider/evidence lane agnostic deterministic get-response racing prep | `model_racing=get_response_racing_v1` opt-in through `--model-racing` / `ROBOCLAWS_OPENAI_AGENTS_MODEL_RACING`, default off, no public MCP/profile change | `roboclaws/agents/drivers/openai_agents_live.py`, `scripts/molmo_cleanup/run_live_openai_agents_cleanup.py`, `just/molmo.just`, `tests/unit/agents/test_live_runtime.py`; `./scripts/dev/run_pytest_standalone.sh -q tests/unit/agents/test_live_runtime.py`; `.venv/bin/ruff check roboclaws/agents/drivers/openai_agents_live.py scripts/molmo_cleanup/run_live_openai_agents_cleanup.py tests/unit/agents/test_live_runtime.py`; `.venv/bin/ruff format --check roboclaws/agents/drivers/openai_agents_live.py scripts/molmo_cleanup/run_live_openai_agents_cleanup.py tests/unit/agents/test_live_runtime.py` | accepted-deterministic-prep, no live speed claim | Deterministic proof shows SDK `get_response` calls can race two or more same-provider arms, return the first successful SDK model response, cancel pending losers, record winner/cancel/failure usage-availability telemetry through D, and keep raw prompts/model text/tool payload bodies out of events. All-arm failure preserves model-service retry/failure behavior. `stream_response` remains single-arm and is labeled `stream_response_single_arm_no_racing` in metrics. | Next C proof is a capped live A/B under the standing authorization only after Q confirms the model/SDK gap remains dominant and live caps record max rows, wall-clock/context limits, concurrency, and racing multiplier. |
