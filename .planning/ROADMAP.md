...
Phase 79 adds placement scene diagnostics to the shared planner report. The
...
Phase 82 classifies that downstream blocker in proof-result summaries and
...
Phase 86 renders that consumed prior proof evidence in runner reports, keeping
prior diagnostics, proof paths, and planner-view images visible before new proof
commands are reviewed.
Phase 87 executed the remaining selected exact-scene candidate and proved it is
also grasp-infeasible, so the next slice must generate or discover a different
candidate rather than retrying the known blocked source requests.
...
Phase 90 executes those selected broader candidates with RBY1M/CuRobo warmup
...
Phase 92 adds prior covered-proof memory so broader selection excludes prior
results that are already `planner_backed` and cleanup-binding promoted. The
...
Phase 94 makes broader source-pool rotation real: generated-mess selection now
uses the MolmoSpaces subprocess seed to choose different eligible objects, and
proof-selection memory no longer lets local `proof_###` / `observed_###`
...
is historical under `.planning/milestones/v1.98-phases/02.4-view-experiment-ab/`:
plans `02.4-01` through `02.4-03` are complete, the cloud-safe analysis script
for `02.4-04` is implemented, and the old multi-variant sweep is superseded by
the `map-v2+chase` decision lock. Phase 2.5 was
...
- 🗄️ **v1.98 Planning Archive** - Phases 2.4-136 archive boundary (archived 2026-05-19; completed/superseded phase dirs moved to `.planning/milestones/v1.98-phases/`, historical pre-GSD plans moved to `docs/retrospectives/plans/`)
...
**Status**: SUPERSEDED 2026-04-21 by Phase 2.6. Plans were drafted in `docs/retrospectives/plans/phase-2.5-autonomous-nav.md` and the 8 plan files exist under `.planning/milestones/v1.98-phases/02.5-.../`, but none were executed. Kept as a lesson — **do not resurrect**.
...
**Source**: `.planning/milestones/v1.98-phases/02.6-openclaw-mcp-tools-integration/02.6-CONTEXT.md` + `02.6-SPIKE-FINDINGS.md`
**Success Criteria** (what must be TRUE):
...
**Status update (2026-04-23):** `roboclaws/regression.py`, `scripts/capture_refactor_regression.py`, `scripts/analyze_refactor_regression.py`, `docs/refactor-regression.md`, and the Phase 4 contract/analyzer tests are implemented. Real local captures now exist for `explore-vlm`, `openclaw-demo`, and `openclaw-autonomous`; the first probe cycle and threshold/harness adjustments are recorded in `.planning/milestones/v1.98-phases/04-refactor-regression-harnesses-for-vlm-territory-coverage-and/04-LOCAL-PROBE-RESULTS.md`.
...
**Depends on**: `docs/retrospectives/plans/molmospaces-manipulation-spike.md` local capability spike (2026-05-07), Phase 2.6 MCP/trace/report lessons, and Phase 4 harness conventions.
...
**Plans**: `.planning/milestones/v1.98-phases/06-molmospaces-api-semantic-cleanup/`
...
`.planning/milestones/v1.98-phases/06-molmospaces-api-semantic-cleanup/06-VERIFICATION.md`.
**UI hint**: no
...
**Plans**: `.planning/milestones/v1.98-phases/07-molmospaces-prompt-driven-cleanup-demo/`
...
`.planning/milestones/v1.98-phases/07-molmospaces-prompt-driven-cleanup-demo/07-VERIFICATION.md`.
**UI hint**: no
...
**Source:** `docs/retrospectives/plans/generic-mcp-entrypoint-semantic-capabilities.md`
