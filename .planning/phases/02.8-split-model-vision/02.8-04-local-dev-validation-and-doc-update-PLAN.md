...
must_haves:
  truths:
    ...
    - "The local evidence includes one direct-vision MiMo control run on the same scene/budget so the phase records whether the split-model path is merely working or actually competitive."
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
    ...
    5. `env -i PATH=\".venv/bin:/usr/bin:/bin\" HOME=$HOME MIMO_TP_KEY=\"$MIMO_TP_KEY\" .venv/bin/pytest tests/test_openclaw_vision_bridge.py tests/test_openclaw_mcp_server.py tests/test_openclaw_bootstrap.py tests/test_openclaw_interactive.py tests/test_tail_openclaw_chat.py tests/test_openclaw_nav_autonomous.py -q`
...
<task type="auto">
  ...
  <action>
    Run these local probes and record every artifact path. Prefer
    `output/probes/02.8/` for new outputs; if an existing helper writes
    elsewhere, record the exact path instead of pretending it did not.
    ...
    1. **Autonomous proof run**
       ...
       Record:
       - `terminated_by`
       - `observe_count` / `move_count`
       - whether `trace.jsonl` contains the bridged observe metadata from Plan 02
       - representative bridge latency from the trace (for example median or
         per-observe summary)
       - whether the run made at least 3 move attempts after a bridged
         `observe`, or reached `terminated_by=done` after a non-trivial
         sequence
       - final `report.html` result

    2. **Direct-vision control run**
       ...
       python examples/openclaw_nav_autonomous.py \
         ...
         --max-moves 30 \
         --wall-budget 180 \
         --output-dir output/probes/02.8/autonomous-mimo-omni-control
       ```

       Record:
       - `terminated_by`
       - `observe_count` / `move_count`
       - representative latency summary
       - the comparison versus the split-model run on the same scene/budget

    3. **Interactive `mimo-pro` proof**
    ...
    4. **Interactive `mimo` proof**

       - repeat the same procedure with `make mimo chat`
       - record the observed `roboclaws__observe` summary line

    5. Write `.planning/phases/02.8-split-model-vision/02.8-LOCAL-PROBE-RESULTS.md`
       with:
       - date and host context
       - exact commands
       - artifact paths
       - autonomous result summary
       - direct-vision control summary
       - split-model vs direct-vision comparison
    ...
    1. Fix the implicated runtime/tooling file(s) in the shipped split-model
       path (for example `roboclaws/openclaw/mcp_server.py`,
       `roboclaws/openclaw/vision_bridge.py`, `scripts/openclaw-bootstrap.sh`,
       `examples/openclaw_interactive.py`, or `scripts/tail-openclaw-chat.py`).
    2. Append a dated correction note to `02.8-SPIKE-FINDINGS.md`.
    3. Update any affected focused tests and re-run the focused pytest slice.
