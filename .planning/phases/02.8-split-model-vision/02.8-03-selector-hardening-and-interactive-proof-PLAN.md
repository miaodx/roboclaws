  ...
  <action>
    ...
    2. In `scripts/tail-openclaw-chat.py`:
       - when `roboclaws__observe` returns text-only content, make that visible
         in the summary line as an explicit delivery mode (for example
         `text-bridge`) instead of reducing it to an opaque parts list
    ...
    3. Update tests:
       ...
       - `tests/test_tail_openclaw_chat.py` covers a text-only `observe`
         toolResult path and asserts the summary is explicit about
         text-bridge delivery
