...
- `roboclaws__snapshot(label)` — (interactive-chat only) write the current FPV, overhead, and chase PNGs to the agent workspace under `./snapshots/` and return their paths. Use this when the operator asks you to show them what you see in the chat tab. Then inline the returned paths in your reply with `MEDIA:./snapshots/<file>.png` — the Control UI renders MEDIA: paths as attachments. Paste the paths EXACTLY as returned. Do NOT use absolute paths or `file://` URLs; they are blocked. `label` is optional; if omitted, a counter is used. **If the Control UI replies "Attachment unavailable" or "Outside allowed folders", STOP** — do not retry with other paths (`/tmp`, `/data`, `~/…`, etc.); the paths returned by the tool are correct and spraying alternatives will not help. Report the error to the operator and wait for guidance.
