...
def test_pinned_coding_agent_docker_toolchain_is_the_ci_source() -> None:
    ...
    assert "Generated by scripts/dev/coding_agent_docker.sh" in docker_script_text
    assert "without host agents, hooks, or skills" in docker_script_text
    assert '-v "${sanitized_codex_home}:/home/agent/.codex"' in docker_script_text
