...
def run_demo(*, output_dir: Path, seed: int = 7, restore_count: int = 5) -> dict[str, Any]:
    ...
    _call_tool(trace_events, started_at, "scene_objects", {}, contract.scene_objects)

    for rule in scenario.private_manifest.targets[:restore_count]:
        target_receptacle_id = rule.valid_receptacle_ids[0]
        _call_tool(
            trace_events,
            started_at,
            "goto",
        ...
        _call_tool(
            ...
            lambda object_id=rule.object_id: contract.pick(object_id),
        )
        _call_tool(
            trace_events,
            started_at,
        ...
        started_at,
        "done",
        {"reason": "scripted reference cleanup complete"},
        lambda: contract.done("scripted reference cleanup complete"),
    )
    ...
    run_result = {
        ...
        "scripted_reference_uses_private_manifest": True,
        "score": done["score"],
...
def main(argv: list[str] | None = None) -> None:
    ...
    result = run_demo(output_dir=args.output_dir, seed=args.seed, restore_count=args.restore_count)
